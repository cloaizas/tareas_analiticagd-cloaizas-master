...
A = FOREACH u GENERATE birthday,
                    SUBSTRING(birthday,8,10) AS fechacompleta,
                    GetDay(ToDate(birthday,'yyyy-MM-dd')) AS dianum,
                    ToString( ToDate(birthday,'yyyy-MM-dd'), 'EE') AS dia_corto,
                    ToString( ToDate(birthday,'yyyy-MM-dd'), 'EEEE' ) AS dia;
